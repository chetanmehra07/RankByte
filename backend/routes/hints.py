...
from services.clerk_auth import (
    verify_clerk_token
)
...
async def get_hint(
    ...
    if user.total_points < HINT_COST:
        raise HTTPException(
            status_code=400,
            detail="Not enough points for a hint"
        )
    ...
    deduct_points(
        ...
        user.id,
        HINT_COST,
        f"Hint #{request.hints_used + 1} for: {challenge.title}"
    )

    return {
        "hint": hint_data.get("hint", ""),
        "hint_number": hint_data.get(
            "hint_number",
            request.hints_used + 1
        ),
        "is_final_hint": hint_data.get(
            "is_final_hint",
            False
        ),
